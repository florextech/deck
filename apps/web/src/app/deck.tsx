...
import { Settings, Zap, Globe, Clipboard, Terminal, Plus, X, AppWindow, ChevronLeft } from "lucide-react";
...
export default function DeckPage() {
  ...
  useEffect(() => {
    ...
    const s: DeckSocket = io(SERVER, { transports: ["polling", "websocket"] });
    setSocket(s);
  ...
  const save = useCallback(async (updated: DeckAction[]) => {
    await fetch(`${getServerUrl()}/config`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ actions: updated }) });
  }, []);
  ...
  return (
    <div className="flex h-dvh flex-col bg-[var(--bg)]">
      ...
      <div className="mx-3 mt-3 flex h-9 items-center gap-2 overflow-hidden rounded-lg bg-[#111114] px-3 font-mono text-xs">
        <div className={`size-1.5 shrink-0 rounded-full ${status === "connected" ? "bg-[var(--green)] animate-pulse" : status === "error" ? "bg-[var(--red)]" : "bg-[var(--yellow)] animate-pulse"}`} />
        <AnimatePresence mode="wait">
          {status !== "connected" ? (
            <motion.span key="status" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 text-[var(--yellow)]">
      ...
      <main className="flex-1 overflow-y-auto p-3">
        ...
        {view === "add-apps" && <AppPicker apps={apps} onPick={addAction} onBack={() => setView("add")} />}
...
function ConfigList({ actions, onRemove, onAdd }: { actions: DeckAction[]; onRemove: (id: string) => void; onAdd: () => void }) {
  return (
    <div className="space-y-2">
      <button onClick={onAdd} className="flex w-full items-center justify-center gap-2 rounded-[var(--radius)] border border-dashed border-[var(--card-border)] bg-transparent py-4 text-sm text-[var(--accent)] hover:border-[var(--accent)] hover:bg-[var(--accent-dim)]">
      ...
      {actions.map((a) => (
        <div key={a.id} className="flex items-center gap-3 rounded-[var(--radius)] bg-[var(--card)] border border-[var(--card-border)] px-4 py-3">
          ...
          <button onClick={() => onRemove(a.id)} className="rounded-lg p-1.5 text-[var(--text-secondary)] hover:text-[var(--red)]"><X size={14} /></button>
        </div>
      ))}
...
function AppPicker({ apps, onPick, onBack }: { apps: DetectedApp[]; onPick: (a: DeckAction) => void; onBack: () => void }) {
  ...
  return (
    <div className="space-y-2">
      <BackBtn onClick={onBack} />
      {apps.length === 0 && <p className="py-8 text-center text-sm text-[var(--text-secondary)]">Agent not connected. Start agent on PC.</p>}
      <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
        {apps.map((app) => (
          <button key={app.name} onClick={() => pick(app)} className="flex flex-col items-center gap-1.5 rounded-[var(--radius)] bg-[var(--card)] border border-[var(--card-border)] p-3 active:bg-[var(--card-hover)] touch-manipulation">
...
function AddForm({ type, onSave, onBack }: { type: "url" | "copy" | "command"; onSave: (a: DeckAction) => void; onBack: () => void }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-3">
      ...
      <button type="submit" className="w-full rounded-[var(--radius)] bg-[var(--accent)] py-3 text-sm font-semibold text-[#0c0c0e] active:opacity-80">Add to deck</button>
    </form>
  );
}
...
function MenuItem({ icon, label, desc, onClick }: { icon: React.ReactNode; label: string; desc: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className="flex w-full items-center gap-3 rounded-[var(--radius)] bg-[var(--card)] border border-[var(--card-border)] px-4 py-3.5 text-left active:bg-[var(--card-hover)] touch-manipulation">
...
function BottomTab({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) {
  return (
    <button onClick={onClick} className={`flex flex-1 flex-col items-center gap-0.5 py-3 ${active ? "text-[var(--accent)]" : "text-[var(--text-secondary)]"}`}>
